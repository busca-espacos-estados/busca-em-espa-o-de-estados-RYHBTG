...
O estado inicial de referencia no `main.py`:
...
Para que o programa funcione de maneira esperada, foi feito a seguinte mudanca para o puzzle:

```python
State((2, 8, 1, 3, 6, 4, 7, 0, 5))
```

é um caso **solucionável** do 8-puzzle clássico com objetivo:
...
- `BFS`, `DFS` e `A*` devem encontrar uma solução, pois ela existe para esse estado inicial.
- `BFS` e `A*` (com heurística admissível, como a distância de Manhattan) encontram o caminho **ótimo** (menor número de movimentos).
- `DFS` encontra **alguma** solução, não necessariamente a de menor custo.
